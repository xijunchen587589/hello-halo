/**
 * JSON-RPC client tests against in-memory streams.
 *
 * The transport contract is line-delimited JSON. We drive the client with
 * a `PassThrough` stdin/stdout pair and assert wire-level behavior:
 *   - outbound requests are framed correctly (no jsonrpc:"2.0" field)
 *   - responses route back to the issuing request
 *   - errors reject with the server's payload
 *   - server requests dispatch to handlers and the handler's return value
 *     is written back as a response sharing the original id
 *   - close rejects every pending request with a transport-closed error
 */
...
function makeClient() {
  ...
  // Capture every line written to stdin so tests can assert the wire format.
...
describe('JsonRpcClient', () => {
  it('frames an outgoing request as a single JSON line with no jsonrpc field', async () => {
    ...
    // Allow the stdin write to flush.
